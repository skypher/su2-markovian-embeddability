...
\begin{abstract}
For a fixed irreducible spin representation, we express the known
isotropic-spin generator classification and Wigner \(6j\) rate transform
in Hilbert--Schmidt normalized channel coordinates.  A covariant channel
with spectrum \(\bm\eta=(\eta_1,\ldots,\eta_{2j})\) on the nontrivial tensor
sectors is a finite-time endpoint of a covariant quantum Markov semigroup if
and only if every component is positive and
\((M^{(j)})^{-1}\log\bm\eta\geq0\).  We derive the determinant
magnitude of \(M^{(j)}\), logarithmic endpoint inequalities through spin
\(2\), and the exact volume of the embeddable region for every spin.  For
identity-normalized invertible differentiable covariant channel families,
the same cone characterizes CP-divisibility through the instantaneous
logarithmic derivatives of the sector eigenvalues.  For the broader class of
nonidentity-normalized channel families specified below, the statement uses
\(\log|\bm\eta|\).
Exact symbolic certificates accompany the low-spin matrices, polytopes,
and volumes.
...
\(\SU\)-covariant GKSL generator; no assertion is made about a
noncovariant logarithm of the same endpoint.  The argument uses three
known inputs, which we restate and verify in the normalization of this
paper:
\begin{enumerate}
 \item the covariant channels form a simplex whose vertices act
 diagonally on irreducible tensor sectors;
 \item every covariant generator is a unique nonnegative combination
 of tensor-rank dissipators; and
 \item the multipole interaction strengths and relaxation rates are
 related by mutually inverse Wigner \(6j\) transforms.
\end{enumerate}
...
\begin{enumerate}
 ...
 covariant endpoint embeddability;
 \item the inverse-rate inequalities become explicit logarithmic-cone
 facets, listed through spin \(2\); and
 \item along identity-normalized invertible differentiable covariant
 channel families, the same cone applied to
 \(d(\log\bm\eta(t))/dt\) characterizes CP-divisibility.  For the broader
 nonidentity-normalized class in Definition~\ref{def:cp-divisibility},
 the corresponding statement uses \(d(\log|\bm\eta(t)|)/dt\).
\end{enumerate}
The results derived here beyond the three cited inputs are the logarithmic
endpoint formulation and its low-spin facets, the determinant-magnitude
identity \(\lvert\det M^{(j)}\rvert=2j+1\), exact Euclidean volumes of the
embeddable regions, exact low-spin comparison polytopes, and the
instantaneous channel-family criterion.  This description separates source
provenance from any claim of historical priority for consequences implicit
in the earlier rate formulas.  All
displayed low-spin matrices and polytope intersections have symbolic
certificates.  Floating-point values are used only to render the
spin-\(1\) boundary curves and spin-\(3/2\) boundary meshes whose defining
equations are exact.
...
Both Python entry points accept \texttt{-h} and \texttt{--help} and exit
...
\eqref{eq:M-inverse-formula}, while the additional determinant-magnitude
identity and componentwise exponential turn this rate cone into an
endpoint criterion.  For identity-normalized channel families,
time-dependent CP-divisibility is the pointwise version of the same test
applied to the logarithmic derivatives of the sector eigenvalues.  For the broader
nonidentity-normalized class in Definition~\ref{def:cp-divisibility}, the
signs are constant and the criterion uses componentwise absolute values.
The
...
repository}.  The earlier review snapshot is archived under the
\href{https://github.com/skypher/su2-markovian-embeddability/tree/v1.1.0}
{\texttt{v1.1.0} tag}; subsequent revisions are identified by their
repository commits.  Machine-readable citation
metadata and the
