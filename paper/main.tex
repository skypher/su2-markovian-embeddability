...
\hypersetup{
  ...
  pdftitle={Embedding SU(2)-Covariant Spin Channels into SU(2)-Covariant Quantum Markov Semigroups},
...
\title{Embedding \(\mathrm{SU}(2)\)-covariant spin channels into\\
\(\mathrm{SU}(2)\)-covariant quantum Markov semigroups}
...
\newcommand{\shorttitle}{EMBEDDING SU(2)-COVARIANT SPIN CHANNELS}
...
\begin{abstract}
We characterize which channels on a fixed irreducible spin space are
finite-time endpoints of \(\mathrm{SU}(2)\)-covariant quantum Markov
semigroups.  Let \(M^{(j)}\) take nonnegative generator rates to the
generator's eigenvalues on tensor sectors.  A covariant channel with nontrivial
sector spectrum \(\bm\eta\) is such an endpoint if and only if every
\(\eta_\ell>0\) and \((M^{(j)})^{-1}\log\bm\eta\geq0\).
We obtain this formulation from the classical isotropic-spin generator
classification and Wigner \(6j\) rate transform, expressed in
Hilbert--Schmidt normalized coordinates.  We derive explicit logarithmic
inequalities through spin \(2\), the identity
\(\lvert\det M^{(j)}\rvert=2j+1\), and the embeddable-region volume
\((2j+1)/(2^{2j}(2j)!)\) in sector-eigenvalue coordinates for every spin.
For invertible differentiable covariant channel families starting at the
identity, the same cone characterizes CP-divisibility through instantaneous
logarithmic derivatives.  For families that need not start at the identity,
the criterion uses logarithms of the absolute sector eigenvalues.
Exact symbolic certificates and reproducible plots accompany the low-spin
matrices, comparison polytopes, and volumes.

\vskip5pt
\noindent\textbf{Keywords.}\enspace \(\mathrm{SU}(2)\)-covariant quantum channels;
quantum Markov semigroups; GKSL generators; Markovian embedding; Wigner
\(6j\) symbols; CP-divisibility.

\vskip5pt
\noindent\textbf{MSC2020 Classification.}\enspace 81P47; 81S22; 81R05.
\end{abstract}
...
\cite[Eqs.~(2)--(5)]{WolfEisertCubittCirac2008}.  Symmetry changes that problem
substantially when it forces both the channel and the generator to be
scalar on a multiplicity-free decomposition.

Here covariance is required of the generator as well as its endpoint:
``embeddable'' always means embeddable by an \(\SU\)-covariant GKSL
generator.  We use the classical generator classification and inverse
relaxation-rate transform to formulate an endpoint test in channel
eigenvalues (Theorem~\ref{thm:endpoint-criterion}), give explicit
inequalities through spin \(2\), and derive the volume of the embeddable
region for every spin (Proposition~\ref{prop:embeddable-volume}).
The corresponding instantaneous test characterizes invertible
CP-divisible covariant channel families
(Theorem~\ref{thm:path-criterion}).  The endpoint test makes no assertion
about noncovariant logarithms of the same channel.

For irreducible \(\SU\) representations, the static channel geometry
is explicit.  Al Nuwairan introduced a family called the EPOSIC channels
and proved that they are precisely
...
\cite[Sec.~1]{AschieriRubaSolovej2024}.
Rudnicki, Klimov, Mu{\~n}oz, Leuchs, and S{\'a}nchez-Soto give a recent
Choi--Kraus characterization of the static \(\SU\)-covariant channel
family and its diagonal action on irreducible tensor sectors
\cite[Secs.~4--5, especially Eq.~(38)]
{RudnickiKlimovMunozLeuchsSanchezSoto2026}.  At each simplex vertex,
...
\cite[p.~1803]{VerriGorini1978}.  Gorini, Verri, and Sudarshan subsequently
...
More generally, Holevo's compact-group standard-form theorem shows that the
completely positive part in a representation of a norm-continuous covariant
semigroup may be chosen covariant and its Hamiltonian may be chosen to
commute with the symmetry representation
\cite[Sec.~3.3.4, Theorem~3.3.5]{Holevo2001}; see also Holevo's original
article \cite[p.~211]{Holevo1993}.  In the finite-dimensional irreducible spin
setting, the multiplicity-free adjoint decomposition resolves this
covariant completely positive part into one scalar rate per tensor rank.
We give a self-contained derivation in the normalization needed for the
endpoint calculation.
Further structural treatments of covariant Markovian evolutions and
uniformly continuous covariant quantum Markov semigroups are given in
\cite[p.~1812]{Holevo1996} and
\cite[Secs.~3--5]{GinattaSassoUmanita2019}; their broader
representation-theoretic settings provide context for the finite-dimensional
single-spin specialization considered here.

Recent work on Weyl dynamical
maps gives semigroup and non-Markovianity criteria in a different
covariant family \cite[Proposition~2]{XuJagadish2026}.
In the same Weyl-covariant setting, Shahbeigi, Amaro-Alcal{\'a},
Pucha{\l}a, and \.{Z}yczkowski describe the log-convex geometry and
volume of accessible channels
\cite[Corollary~4, Sec.~IV, and Eq.~(68)]{ShahbeigiEtAl2021}.

For time-dependent evolutions, the distinction between semigroup
embeddability and divisibility is developed in
\cite[Secs.~2--4]{WolfCirac2008} and \cite[Sec.~3]{DavalosZimanPineda2019}.
The relation between CP-divisibility and time-local GKSL rates is treated in
\cite[especially Eqs.~(1)--(2)]{RivasHuelgaPlenio2010},
\cite[Secs.~II--III]{HallCresserLiAndersson2014}, and
\cite[Secs.~2--3]{MontesCabreraDavalosGorin2019}.  These references use
identity-normalized dynamical maps.  Definition~\ref{def:cp-divisibility}
also permits channel families without a distinguished identity initial
channel; the distinction is stated explicitly there.

The paper uses a single Hilbert--Schmidt normalization throughout.
Sections~2--4 develop the tensor conventions, covariant generators, and
Wigner \(6j\) eigenvalue matrix and its inverse.  Section~5 gives the
endpoint criterion and low-spin inequalities.  Section~6 compares the
channel regions and derives the general volume formula; Section~7 treats
CP-divisible families.  Section~8 describes the exact reconstruction and
geometry certificates.  Floating-point data are used only for rendering
the curves, boundary meshes, and interior samples.
...
\begin{definition}[Dynamical conventions]\label{def:dynamical-conventions}
A channel on a finite-dimensional matrix algebra is a complex-linear,
completely positive, trace-preserving map; all channels below are written
in the Schr\"odinger picture.  A linear map \(A:B(\mathcal H_j)\to
B(\mathcal H_j)\) is \(\SU\)-covariant when
\begin{equation}\label{eq:covariance-definition}
 A\circ\cU_g=\cU_g\circ A\qquad(g\in\SU).
\end{equation}
A norm-continuous quantum Markov semigroup is a family
\(\{e^{t\cL}\}_{t\geq0}\) of channels satisfying the semigroup law.
Its generator \(\cL\) is covariant when it satisfies
\eqref{eq:covariance-definition}, equivalently when every
\(e^{t\cL}\) is covariant.
\end{definition}

Throughout, column vectorization means
...
=(A\otimes\overline B)\operatorname{vec}(X)\).
...
\begin{definition}[Covariant rank dissipators]\label{def:dissipators}
...
\section{Classification of covariant generators}

\begin{theorem}[Covariant-generator decomposition]
...
\cite[Sec.~3.3.4, Theorem~3.3.5]{Holevo2001}.  The proof below identifies the unique
...
basis of the traceless operators.  Although the spherical tensors need
not be self-adjoint, they are related to any self-adjoint
Hilbert--Schmidt orthonormal basis by a unitary basis change.  Under that
change the GKS matrix is conjugated unitarily, so its positivity and the
uniqueness of the canonical traceless data are preserved.  The canonical
finite-dimensional Gorini--Kossakowski--Sudarshan--Lindblad representation,
as stated in
\cite[Theorem~2.2 and Lemma~2.3]{GoriniKossakowskiSudarshan1976}
(see also \cite[Theorem~2]{Lindblad1976}), gives unique data
...
In the tensor basis, denote the matrix of this representation by
\begin{equation*}
 D(g)=\bigoplus_{k=1}^{2j}D^{(k)}(g),
\end{equation*}
as fixed by \eqref{eq:tensor-transform}.
...
\end{equation}
Under the column-vectorization convention
\eqref{eq:column-vectorization}, the Kraus map in
...
the rotation-by-\(\pi\) matrix about the \(y\)-axis in
...
In the chosen angular-momentum basis, the Clebsch--Gordan coefficients
and hence the tensors are real, and
...
\cite[Eq.~(2.14)]{Breuer2005}; see also
\cite[Sec.~34.5(iv)]{NISTDLMF}.  It says that \(L\) is orthogonal.
The tetrahedral symmetry in \cite[Sec.~34.5(ii)]{NISTDLMF} and
...
\end{equation*}
The lower-right block of \(V^{-1}\) is therefore \(B^{-1}\).  Moreover,
...
\(r=(M^{(j)})^{-1}\log\bm\eta\geq0\) and, taking \(t=1\),
...
\begin{corollary}[General logarithmic-cone facets and multiplicative geometry]
...
unique integrated-rate vector: for an expression
\(\Phi=e^{t\sum_k\gamma_k\cD_k}\), it is \(r=t\bm\gamma\).
The time \(t>0\) and rate vector \(\bm\gamma\) separately retain the
scaling freedom \((t,\bm\gamma)\mapsto(at,\bm\gamma/a)\), \(a>0\).
In logarithmic eigenvalue coordinates,
...
Both preserve \(\Rplus^{2j}\).  Equivalently, all covariant generators
...
\centering\small
\begin{tabular}{c p{1.55in} p{3.05in}}
\hline
symbol & role & defining relation \\ \hline
\(p_k\) & channel-simplex weight
  & \(\Phi=\sum_kp_k\cE_k\), \(p_k\geq0\), \(\sum_kp_k=1\) \\
\(\alpha_k\) & static channel weight
  & \(p_k=(2k+1)\alpha_k\)
    \cite[Eqs.~(26)--(31)]{RudnickiKlimovMunozLeuchsSanchezSoto2026} \\
\(\gamma_k\) & generator rate
  & \(\cL=\sum_{k\geq1}\gamma_k\cD_k\), \(\gamma_k\geq0\) \\
\(\lambda_\ell\) & generator eigenvalue on \(V_\ell\)
  & \(\bm\lambda=M^{(j)}\bm\gamma\) \\
\(\eta_\ell\) & channel eigenvalue on \(V_\ell\)
  & \(\eta_\ell=e^{t\lambda_\ell}\) for a semigroup endpoint \\
\(r_k\) & integrated generator rate
  & \(\bm r=(M^{(j)})^{-1}\log\bm\eta=t\bm\gamma\) \\
\(\chi_\ell\) & historical sector decay rate
  & \(\chi_\ell=-t^{-1}\log\eta_\ell\) \\ \hline
\end{tabular}
\captionsetup{hypcap=false}
\captionof{table}{Weight, rate, and sector-eigenvalue conventions used in the paper.}
\label{tab:notation-dictionary}
\end{minipage}
\end{center}
...
general covariant semigroup is the product of these commuting
processes.
\end{proposition}
...
\section{Channel geometry and exact volumes}

\subsection{Low-spin channel polytopes}
...
\begin{proposition}[Computer-assisted exact comparison polytopes]
\label{prop:comparison-polytopes}
...
\(\eta=V\bm p\), where the columns of \(V\) are the displayed
simplex vertices and \(p_k\geq0\), \(\sum_kp_k=1\).
...
\(p_k=0\) and \(\eta_\ell=0\).  The exact enumeration in
...
For direct inspection, the complete list of feasible full-rank active
sets is as follows.  The \(p_k\) are the barycentric coordinates in the
order of the simplex vertices displayed above.
For spin \(1\),
\begin{equation*}
\begin{array}{c|l}
 \text{vertex}&\text{active equalities}\\
 \hline
 (0,0)&\eta_1=0,\ \eta_2=0\\
 (0,2/5)&p_1=0,\ \eta_1=0\\
 (2/3,0)&p_2=0,\ \eta_2=0\\
...
\begin{array}{c|l}
 \text{vertex}&\text{active equalities}\\
 \hline
 (0,0,0)&\eta_1=0,\ \eta_2=0,\ \eta_3=0\\
 (0,0,5/21)&p_1=0,\ \eta_1=0,\ \eta_2=0\\
 (0,1/3,0)&p_2=0,\ \eta_1=0,\ \eta_3=0\\
 (0,1/2,5/14)&p_1=0,\ p_2=0,\ \eta_1=0\\
 (1/2,0,1/2)&p_1=0,\ p_3=0,\ \eta_2=0\\
 (5/9,0,0)&p_3=0,\ \eta_2=0,\ \eta_3=0\\
 (5/6,1/2,0)&p_2=0,\ p_3=0,\ \eta_3=0\\
 (1,1,1)&p_1=0,\ p_2=0,\ p_3=0.
\end{array}
\end{equation*}
The generated certificate records the same active equalities and the
corresponding barycentric coordinates.  Every other full-rank selection
is either singular or violates at least one of the halfspaces.
\end{proof}

\subsection{Embeddable volume for arbitrary spin}
...
\(T_0^{(k)}\).  The negative scalar term in
\eqref{eq:dissipator-simplified} contributes \(-(2k+1)\) to the
restricted trace, so
...
change-of-variables theorem on the interior loses no volume.  Apply that
theorem first to the bounded boxes \((0,R)^n\).  Their images increase to
the image of the open orthant, and the nonnegative Jacobian integrands
increase by indicator functions; monotone convergence therefore gives
the integral over \(\mathbb R_{>0}^n\).  Adding the null boundary and using
...
\subsection{Volume comparisons and figures}

Table~\ref{tab:volume-comparison} collects the low-spin volumes and
fractions from Proposition~\ref{prop:embeddable-volume}.  All volumes use
Lebesgue measure in the sector-eigenvalue coordinates.
\begin{center}
\begin{minipage}{\textwidth}
\centering\small
\begin{tabular}{c c c c c c}
\hline
Spin & Full simplex & Nonnegative & Embeddable
 & \multicolumn{2}{c}{Embeddable fraction of}\\
 & \(\mathsf S_j\) & \(\mathsf S_j^+\) & \(\mathsf M_j\)
 & \(\mathsf S_j\) & \(\mathsf S_j^+\)\\ \hline
\(1\) & \(9/10\) & \(8/15\) & \(3/8\) & \(5/12\) & \(45/64\)\\
\(3/2\) & \(128/315\) & \(25/126\) & \(1/12\) & \(105/512\) & \(21/50\)\\ \hline
\end{tabular}
\captionsetup{hypcap=false}
\captionof{table}{Exact volumes and embeddable fractions for spins
\(1\) and \(3/2\).}
\label{tab:volume-comparison}
\end{minipage}
\end{center}

Figure~\ref{fig:spin-one-regions} and
Figures~\ref{fig:spin-three-halves-polytopes}--\ref{fig:spin-three-halves-regions}
compare these polytopes with the
...
exact.  For spin \(3/2\), the rendered boundary meshes truncate the
nonnegative rates at \(8\), while an interior point cloud samples strictly
positive rates up to \(4\); the inequalities
...
\section{Differentiable CP-divisible channel families}

\begin{definition}[CP-divisible channel families]\label{def:cp-divisibility}
Let \(I\subset\mathbb R\) be a nondegenerate interval.  A family of
channels \(\{\Phi_t\}_{t\in I}\) is a CP-divisible channel family if there
are channels
\(\cV_{t,s}\) for every \(s\leq t\) in \(I\) such that
\begin{equation}\label{eq:propagator-convention}
 \Phi_t=\cV_{t,s}\Phi_s,\qquad
 \cV_{t,t}=\id,\qquad
 \cV_{u,t}\cV_{t,s}=\cV_{u,s}\quad(s\leq t\leq u).
\end{equation}
When \(I\) has a least element \(t_0\) and \(\Phi_{t_0}=\id\), we call the
family identity-normalized; this is the dynamical-map convention used in
the references cited above.  Definition~\ref{def:cp-divisibility} also
allows a broader channel family without a distinguished identity element.
For the invertible families
considered below, the first identity in
\eqref{eq:propagator-convention} uniquely determines the propagators, and
the other two identities follow.  Thus fixed negative sector signs can
occur only in the broader, nonidentity-normalized class.
A \(C^1\) channel family on \(I\) below has a continuous derivative in the
interior, with the corresponding one-sided continuous derivative at
each included endpoint.  Norms of superoperators, including those in
remainder and convergence statements, are the operator norms induced
by the Hilbert--Schmidt norm.
\end{definition}

The next lemma uses Choi's matrix criterion for complete positivity
\cite[Theorem~2]{Choi1975} to describe the tangent cone at the identity,
written in the present vectorization convention.  We include the
finite-dimensional derivation to make the normalization explicit.
...
\begin{proof}
Choose a Hilbert--Schmidt orthonormal basis satisfying
\begin{equation*}
 F_0=I/\sqrt d,\qquad \Tr F_a=0\quad(1\leq a\leq d^2-1).
\end{equation*}
For \(A\in B(\mathbb C^d)\), write
...
Put \(e_0=\Omega/\sqrt d\).  Relative to
\(\mathbb Ce_0\oplus\Omega^\perp\), there are \(a\in\mathbb R\) and
\(z\in\Omega^\perp\) such that
\begin{equation*}
 J(R)=a|e_0\rangle\langle e_0|
      +|z\rangle\langle e_0|+|e_0\rangle\langle z|.
\end{equation*}
Choose \(A\in B(\mathbb C^d)\) by
\begin{equation*}
 |A\rangle\!\rangle=\frac{a}{2\sqrt d}|e_0\rangle
                      +\frac1{\sqrt d}|z\rangle.
\end{equation*}
...
finite-dimensional matrix algebra.  It is a CP-divisible channel family
if and only if
...
\begin{proof}
If the family is CP-divisible, invertibility makes the propagator unique:
\(\cV_{t+h,t}=\Phi_{t+h}\Phi_t^{-1}\).  It is a channel and
\begin{equation*}
 \cV_{t+h,t}=\id+hK_t+o(h).
\end{equation*}
...
\(\psi\perp\Omega\).  Since \(J(\cV_{t+h,t})\geq0\),
\begin{equation}\label{eq:choi-tangent-condition}
 0\leq \langle\psi,J(\cV_{t+h,t})\psi\rangle
...
\(\partial_t\cV_{t,s}=K_t\cV_{t,s}\), \(\cV_{s,s}=\id\).  Indeed, if
...
Hence \(\cV_{t,s}\) is a channel.  Finally, both \(\Phi_t\) and
\(\cV_{t,s}\Phi_s\) solve \(\dot Y_t=K_tY_t\) with initial value
...
Let \(t\mapsto\Phi_t\) be an invertible \(C^1\) family of
\(\SU\)-covariant channels, with tensor-sector eigenvalues
\(\eta_\ell(t)\).
If the family is identity-normalized at its least time \(t_0\), then every
\(\eta_\ell(t)>0\), and it is CP-divisible if and only if
\begin{equation}\label{eq:path-cone-identity}
 (M^{(j)})^{-1}\frac{d}{dt}
 \begin{pmatrix}
...
\end{equation}
More generally, without identity normalization, it is a CP-divisible
channel family in the sense of Definition~\ref{def:cp-divisibility} if and
only if
...
\begin{corollary}[Spin-\(1/2\) fixed-sign channel families]
\label{cor:negative-spectrum-paths}
Let \(t\mapsto\Phi_t\) be an invertible \(C^1\) family of spin-\(1/2\)
covariant channels.  Then it is a CP-divisible channel family if and only
if
\begin{equation*}
...
reduces to the displayed inequality.
\end{proof}

\begin{remark}
Under Definition~\ref{def:cp-divisibility}, every constant invertible
channel family is CP-divisible, with identity propagators.  In particular,
this applies to the spin-\(1/2\) nonidentity vertex \(\eta_1=-1/3\).  The
example illustrates the absence of identity normalization in the broader
channel-family convention; it does not embed that negative-spectrum vertex
into a time-homogeneous quantum Markov semigroup issuing from the identity.
\end{remark}
...
normalization checks explicit.  Both constructions use SymPy; its
Clebsch--Gordan, \(3j\), and \(6j\) routines share library infrastructure.

\begin{remark}[Exact reconstruction protocol]
...
The tensor and Wigner routines return SymPy algebraic numbers
\cite{MeurerEtAl2017}, and the tests use exact matrix equality after
symbolic simplification.  The direct Kraus reconstruction does not call
the \(6j\)-based generator routine, and the Choi matrix-unit loop is
separate from the vectorized-projector construction.  The executable
check, run from the repository root, is
\begin{center}
 \verb|python3 -u code/exact_cones.py 1 2 3 4 --verify|
\end{center}
\end{remark}

\begin{remark}[Exact polytope certificates]
...
the rationals, and records each feasible vertex, its active equalities,
its barycentric coordinates, the exact polytope volumes, and the
embeddable-region Jacobian volumes in
\texttt{paper/generated/geometry\_exact.txt}.
The only floating-point data produced by the script are plotting
coordinates: the spin-\(1\) curves, the truncated spin-\(3/2\)
boundary meshes, and the interior point cloud.
...
Both Python entry points accept \verb|-h| and \verb|--help| and exit
without generating or changing artifacts.

\section{Conclusion}

The classical isotropic-spin generator and relaxation-rate transform
provides an explicit endpoint test in channel eigenvalues: positivity of
the spectrum and membership of its logarithm in the generator cone.
The determinant identity \(\lvert\det M^{(j)}\rvert=2j+1\) and the
column sums of this transform give the exact embeddable-region volume
\((2j+1)/(2^{2j}(2j)!)\) in sector-eigenvalue coordinates.
The low-spin inequalities, volume comparisons, and plots distinguish
complete positivity, nonnegative sector spectrum, and covariant
semigroup embeddability.  For invertible differentiable families starting
at the identity, CP-divisibility is characterized by the same cone
applied to instantaneous logarithmic derivatives; Theorem~\ref{thm:path-criterion}
also covers families without an identity initial channel.
...
The source, exact verification scripts, generated rational certificates,
plotting data, and build instructions are available in the
\href{https://github.com/skypher/su2-markovian-embeddability}{project
repository}.  The source and artifacts corresponding to this submission
are archived under the
\href{https://github.com/skypher/su2-markovian-embeddability/tree/v1.2.0}
{\texttt{v1.2.0} tag}.  Machine-readable citation
metadata and the
repository reuse terms are recorded in \texttt{CITATION.cff} and
\texttt{LICENSE}, respectively.
The saved
...
\begin{center}
\small\texttt{a556b931d879e9dde4e8fa52a0ae3b70c2299afbde37734be48a798f2137207f}.
\end{center}
