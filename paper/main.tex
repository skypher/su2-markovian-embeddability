...
with nontrivial sector spectrum \(\bm\eta\) is a finite-time endpoint of a
covariant quantum Markov semigroup if and only if every component is
positive and \((M^{(j)})^{-1}\log\bm\eta\geq0\).  We derive the determinant
...
identity-normalized invertible differentiable covariant channel paths,
the same cone characterizes CP-divisibility through the instantaneous
logarithmic derivatives of the sector eigenvalues.  The statement extends,
with \(\log|\bm\eta|\), to channel paths not normalized at the identity.
...
also permits paths whose initial channel is not the identity; the distinction
is stated explicitly there.
...
\begin{enumerate}
 ...
 paths, the same cone applied to \(d(\log\bm\eta(t))/dt\) characterizes
 CP-divisibility.  For paths with a nonidentity initial channel, the
 corresponding statement uses \(d(\log|\bm\eta(t)|)/dt\).
\end{enumerate}
The additional calculations are the determinant-magnitude identity
\(\lvert\det M^{(j)}\rvert=2j+1\), exact Euclidean volumes of the
embeddable regions, and exact low-spin comparison polytopes.  All
...
\begin{definition}[CP-divisible channel paths]\label{def:cp-divisibility}
Let \(I\subset\mathbb R\) be a nondegenerate interval.  A family of
channels \(\{\Phi_t\}_{t\in I}\) is CP-divisible if there are channels
...
\end{equation}
The channel-path definition does not itself require
\(\Phi_{t_0}=\id\) at any distinguished time.  When this equality holds,
we call the path identity-normalized at \(t_0\).  For the invertible paths
...
occur only under the broader, nonidentity-normalized convention.
A \(C^1\) path on \(I\) below has a continuous derivative in the
...
\begin{corollary}[Spin-\(1/2\) fixed-sign paths]
...
channel path is CP-divisible, with identity propagators.  In particular,
...
path convention; it does not embed that negative-spectrum vertex into a
time-homogeneous quantum Markov semigroup issuing from the identity.
\end{remark}
...
endpoint criterion.  For identity-normalized paths, time-dependent
CP-divisibility is the pointwise version of the same test applied to the
logarithmic derivatives of the sector eigenvalues.  For the broader
channel-path convention, the signs are constant and the criterion uses
the componentwise absolute values.  The
...
repository}.  This reviewed manuscript revision is archived under the
\href{https://github.com/skypher/su2-markovian-embeddability/tree/v1.1.0}
{\texttt{v1.1.0} tag}.  Machine-readable citation
metadata and the
