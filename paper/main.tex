...
\begin{enumerate}
 ...
 branches.
\end{enumerate}
The static channel simplex and the general covariant standard form are
the starting points supplied by the cited literature.  The present
contribution identifies their common tensor normalization, derives the
closed inverse and determinant of the generator eigenmatrix, determines
the exponential endpoint cone and its low-spin facets, and proves the
fixed-sign CP-divisibility criterion.  It also gives exact volume and
polytope certificates.  All displayed low-spin matrices and polytope
intersections have exact symbolic certificates; floating-point values
are used only to render sampled boundary curves and meshes.
...
under conjugation.  The tensors are real, and
...
selection rule in \eqref{eq:tensor-definition} says that
\(T_q^{(k)}\) has a diagonal entry only when \(q=0\).  Hence the Kraus
map
\begin{equation*}
...
\end{equation*}
Thus \(K_t\) is conditionally completely positive and conservative.
The conditional-complete-positivity construction in the proof of the
finite-dimensional GKS theorem
\cite[Theorem~2.2]{GoriniKossakowskiSudarshan1976} gives its GKSL
form.  At a right endpoint of \(I\), the same conclusion follows by
...
\begin{enumerate}
 ...
 \item constructs each \(J(\cE_k)\) independently from matrix units
 and from the tensor vectorizations, tests
...
construction.  Thus items 1--4 compare independent constructions entry
by entry; item 5 substitutes the resulting matrices into the displayed
algebraic identities.  The executable check is
\begin{equation*}
...
The low-spin corollaries and comparison plots make the distinction
between complete positivity, sectorwise nonnegativity, and covariant
Markovian embeddability explicit.
