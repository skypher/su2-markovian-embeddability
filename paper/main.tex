...
characterizes the instantaneous logarithmic derivative of an invertible
differentiable covariant CP-divisible path.
\end{abstract}
...
degradability questions are analyzed, in particular in Sec.~5.
...
\begin{enumerate}
 ...
 \(d(\log\bm\eta(t))/dt\) characterize invertible differentiable
...
\section{The exact \(6j\)-symbol matrix}
...
Conversely, if every \(K_t\) is a GKSL generator, the solution of
\(\partial_tV_{t,s}=K_tV_{t,s}\), \(V_{s,s}=\id\), is the norm limit
of products
\begin{equation*}
 \prod_{r=0}^{n-1}\exp\!\left(\frac{t-s}{n}
 K_{s+r(t-s)/n}\right).
\end{equation*}
Every factor is a channel, and the cone of completely positive maps
and the trace-preserving affine subspace are norm closed.  Hence
\(V_{t,s}\) is a channel and \(\Phi_t=V_{t,s}\Phi_s\).
\end{proof}

\begin{theorem}[Instantaneous logarithmic cone]
\label{thm:path-criterion}
Let \(t\mapsto\Phi_t\) be an invertible \(C^1\) path of
\(\SU\)-covariant channels whose tensor-sector eigenvalues
\(\eta_\ell(t)\) are positive.  Then the path is CP-divisible if and
only if
...
For \(2j=1,2,3,4\), the script
\texttt{code/exact\_cones.py} performs all of the following operations
...
\begin{enumerate}
 ...
 \item applies every Kraus sum directly to \(T_0^{(\ell)}\) and tests
 that the resulting matrix equals \eqref{eq:M-definition};
...
routine.  Thus items 1--3 compare the two derivations entry by entry;
item 4 substitutes the resulting matrices into the displayed algebraic
identities.  The executable check is
\begin{equation*}
...
the inverse augmented vertex matrix.  This is the active-set argument
used in Proposition~\ref{prop:comparison-polytopes}.
\end{proof}
...
of the same test.  The low-spin corollaries and comparison plots make
