...
\begin{abstract}
For a fixed irreducible spin representation, we translate the known
isotropic-spin generator classification and Wigner \(6j\) rate transform
into Hilbert--Schmidt normalized channel coordinates.  Building on those
inputs, we compute the determinant of the generator eigenvalue matrix,
formulate the exponential endpoint criterion, obtain logarithmic endpoint
inequalities through spin \(2\), and derive the exact volume of the
embeddable region for every spin.  Under an explicitly identity-free
channel-path convention, the same cone characterizes the instantaneous
logarithmic derivatives of the componentwise absolute values of the
tensor-sector eigenvalues along any invertible differentiable covariant
CP-divisible path.  Exact symbolic certificates accompany the low-spin
matrices, polytopes, and volumes.
...
\cite[Sec.~3.3.4, Theorem~3.3.5]{Holevo2001}.  In the finite-dimensional irreducible
...
For time-dependent evolutions, CP-divisibility and its relation to
time-local GKSL rates are developed in
\cite[especially Eqs.~(1)--(2)]{RivasHuelgaPlenio2010} and
\cite[Secs.~II--III]{HallCresserLiAndersson2014}.  Those works use the
usual identity-based dynamical-map setting.  Definition~\ref{def:cp-divisibility}
below states the broader channel-path convention used here; the
identity-based case is then recovered as a corollary.
...
noncovariant logarithm of the same endpoint.  For completeness, the
paper first rederives in the fixed normalization two inputs from the
cited literature:
\begin{enumerate}
 \item every covariant generator is a unique nonnegative combination
 of the tensor-rank dissipators \(\cD_k\);
 \item its sector eigenvalue matrix \(M^{(j)}\) and inverse matrix are
 single \(6j\)-symbol formulas.
\end{enumerate}
The additional results developed here are:
\begin{enumerate}
 \item the determinant identity \(\lvert\det M^{(j)}\rvert=2j+1\);
 \item positivity of all sector eigenvalues together with
 \((M^{(j)})^{-1}\log\bm\eta\geq0\) is necessary and sufficient for
 covariant endpoint embeddability, yielding explicit logarithmic-cone
 facets through spin \(2\);
 \item the exact Euclidean volume of the embeddable region and exact
 low-spin comparison polytopes;
 \item the same inequalities applied to
 \(d(\log|\bm\eta(t)|)/dt\) characterize all invertible differentiable
 covariant CP-divisible channel paths under Definition~\ref{def:cp-divisibility},
 including fixed negative-spectrum branches.
\end{enumerate}
The static channel simplex, its sector eigenvalue transform, and the
isotropic generator--relaxation-rate transform are taken from the cited
literature.  The present contribution places them
...
spin-\(2\) logarithmic-cone facets, proves the fixed-sign CP-divisibility
criterion, and
...
\begin{definition}[CP-divisibility]\label{def:cp-divisibility}
...
\end{equation}
No distinguished identity time is required in this channel-path
definition.  When \(\Phi_{t_0}=\id\), we call the family identity-based
at \(t_0\).  For the invertible paths considered below, the first
identity in \eqref{eq:propagator-convention} uniquely determines the
propagators, and the other two identities follow.
...
sense of Definition~\ref{def:cp-divisibility}, but it is not
identity-based.
\end{proof}
...
the same test applied to the logarithmic derivatives of the
componentwise absolute sector eigenvalues.  The
...
repository}.  The initial public manuscript release is available under the
\href{https://github.com/skypher/su2-markovian-embeddability/tree/v1.0.0}
{\texttt{v1.0.0} tag}; subsequent revisions, including this manuscript,
are identified by the repository history.  Machine-readable citation
metadata and the
